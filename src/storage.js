...
async function fetchFromS3(bucket, key) {
  try {
    // Download from S3
    const record = await s3Client.send(new GetObjectCommand({ Bucket: bucket, Key: key }))
...
async function uploadToS3(bucket, key, content) {
  try {
    await s3Client.send(
      new PutObjectCommand({ Bucket: bucket, Key: key, Body: content, ContentType: 'application/json' })
    )
  } catch (e) {
...
async function publishToSQS(queue, data, additionalAttributes = {}) {
  try {
    await sqsClient.send(new SendMessageCommand({ QueueUrl: queue, MessageBody: data, ...additionalAttributes }))
  } catch (e) {
